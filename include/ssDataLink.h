 ...
 * there were remaining bytes in the chunk.
 */
typedef void (*ssdl_parseFunction_t)( uint8_t* data, uint16_t size );

/**
 * Pointer to the function that SSDL_EncodeAndSend will call with every
 * byte of the input buffer. This function could save the data for
 * sending it later or send it directly over the medium.
 */
 ...
 *
 * A slip coded string is generated byte by byte swiping inLen bytes
 * from inBuff. sender, containing the slip encoded data, is being called
 * with each byte (can be 1 or 2 bytes each time). At the end the CRC and the
